...
ENV *rc_new_env(int size)
{
	...
	if ((p = (ENV *)malloc(sizeof(*p))) == NULL)
		return NULL;

	if ((p->env = (char**)malloc(size * sizeof(char *))) == NULL)
	{
...
int rc_add_env(ENV *env, char *name, char *value)
{
	...
	if (env->env[i])
	{
		if ((new_env = (char*)realloc(env->env[i], strlen(name)+strlen(value)+2)) == NULL)
			return -1;
		...
		if ((env->env[env->size] = (char*)malloc(strlen(name)+strlen(value)+2)) == NULL) {
