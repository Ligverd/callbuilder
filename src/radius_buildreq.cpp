...
unsigned char rc_get_seqnbr(rc_handle *rh)
{
	...
	int tries = 1;
	int seq_nbr;
	char *seqfile = rc_conf_str(rh, (char *)"seqfile");
    ...
    char *msg, int add_nas_port, int request_type)
{
	...
	int		i, skip_count;
	SERVER		*aaaserver;
	int		timeout = rc_conf_int(rh, (char *)"radius_timeout");
	int		retries = rc_conf_int(rh, (char *)"radius_retries");
	int		radius_deadtime = rc_conf_int(rh, (char *)"radius_deadtime");
	double		start_time;
	time_t		dtime;

	
	if (request_type != PW_ACCOUNTING_REQUEST) {
		aaaserver = rc_conf_srv(rh, (char *)"authserver");
	} else {
		aaaserver = rc_conf_srv(rh, (char *)"acctserver");
	}
	if (aaaserver == NULL)
		return ERROR_RC;
