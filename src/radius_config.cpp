...
#include <config.h>
#include <radius_includes.h>
#include <radius_freeradius-client.h>
#include <radius_options.h>
...
rc_config_init(rc_handle *rh)
{
    ...
    authservers = rc_conf_srv(rh, (char *)"authserver"); 
	acctservers = rc_conf_srv(rh, (char *)"acctserver");
...
int rc_find_server (rc_handle *rh, char *server_name, uint32_t *ip_addr, char *secret)
{
	...
	if( (authservers = rc_conf_srv(rh, (char *)"authserver")) != NULL ) 
	{
	...
	if( (acctservers = rc_conf_srv(rh, (char *)"acctserver")) != NULL ) 
	{
	...
	if ((clientfd = fopen (rc_conf_str(rh, (char *)"servers"), "r")) == NULL)
	{
		rc_log(LOG_ERR, "rc_find_server: couldn't open file: %s: %s", strerror(errno), rc_conf_str(rh, (char *)"servers"));
		return -1;
	}
	...
	if (result == 0)
	{
		...
		rc_log(LOG_ERR, "rc_find_server: couldn't find RADIUS server %s in %s",
			 server_name, rc_conf_str(rh, (char *)"servers"));
		return -1;
	}
	return 0;
}
